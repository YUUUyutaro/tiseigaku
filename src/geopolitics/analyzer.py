...
必ず以下のスキーマに従い、JSON オブジェクトのみを出力してください:
{
  ...
  "importance": "高|中|低",
  "region_tags": ["east_asia" | "southeast_asia" | "europe" | "middle_east" | "americas" | "africa" | "cyber" | "gcap" | "defense_export" | "global"],
  "actors": [
...
- cyber: サイバー攻撃・情報戦
- gcap: 日英伊次期戦闘機共同開発
- defense_export: 防衛装備移転三原則、日本からの装備輸出
- global: 複数地域にまたがる案件
...
def _guess_region_tags(article: Article) -> List[str]:
    ...
    if any(kw in text for kw in [
        "防衛装備移転", "武器輸出", "防衛装備", "装備移転三原則",
        "arms export", "defense export", "defence export",
    ]):
        tags.append("defense_export")
